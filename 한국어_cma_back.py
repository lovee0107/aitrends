...
def evaluate_t_list(t_list):
    ...
    print(f"평가 완료: acc_ko = {acc}", flush=True)

    with open(f"results/한국어/{path}_history_log.txt", "a") as log:
        log.write(f"{es.countiter}, acc_ko = {acc}\n")

    last_line = float(last_line)
    if good <= last_line:
        good = last_line
        print("저장하기")
        with open(f"results/한국어/{path}_t_list_good_{good}.json", "w") as f:
...
# CMA-ES 초기화
layer_count = 291
es = cma.CMAEvolutionStrategy([0.33] * layer_count, 0.1)


os.makedirs("results/한국어", exist_ok=True)
with open(f"results/한국어/{path}_cma_stdout.log", "w") as f_out:
...
print("최고 acc_ko =", best_acc)

# 최종 결과 저장
best_t = es.result.xbest.tolist()
os.makedirs("results", exist_ok=True)
with open(f"results/한국어/{path}_cma_best_t_list.json", "w") as f:
